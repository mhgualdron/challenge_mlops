...
def main():
    ...
    base_models = {
        ...
        "XGBoost": XGBRegressor(
            objective="reg:squarederror", n_estimators=100, seed=42, verbosity=0
        ),
    }
        ...
        for model_name, model in base_models.items():
            ...
            with mlflow.start_run(run_name=run_name):
                ...
                if prod_score < best_score:
                    ...
                    best_config = {"model": model_name, "imputer": imp_name}
    ...
    with mlflow.start_run(run_name=champion_name):
        ...
        metrics_to_log = {
            k: v for k, v in best_metrics.items() if v is not None and not np.isnan(v)
        }
        mlflow.log_metrics(metrics_to_log)

        # Register in Model Registry
        mlflow.sklearn.log_model(
            best_pipeline, "model", registered_model_name="boston_housing_champion"
        )
